...
export const apps = [
  ...
  },
  {
    ...
    guide: {
      stack: ['Next.js', 'React', '인라인 스타일'],
      summary: '웨딩 스튜디오를 위한 로즈 톤 홈페이지 데모예요. 세리프 이탤릭 타이틀로 청첩장 같은 감성을 살렸어요.',
      keyPoints: [
        '제목에만 세리프 서체(Georgia)와 이탤릭을 적용해 본문 고딕 폰트와 대비를 줬어요.',
        '패키지 3종 중 하나에 "인기" 배지와 강조 테두리를 줘서 자연스럽게 시선을 유도해요.',
        '갤러리 섹션은 실제 이미지 대신 이모지 그리드로 자리표시자를 채웠어요.',
      ],
      steps: [
        '업종 감성에 맞는 서체 대비(세리프 제목 + 고딕 본문)를 정해요.',
        '패키지 배열에 highlighted 플래그를 하나 넣어 강조 카드를 표시해요.',
        '실제 사진이 없을 때는 이모지나 색상 블록으로 자리표시자를 채워요.',
      ],
      files: ['components/gallery/WeddingLandingDemo.jsx'],
    },
  },
]
