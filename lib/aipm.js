...
export async function mapExcelToTasks({ headers, rows, projectId }) {
  ...
  const prompt = `You are a data migration assistant. Convert the following spreadsheet data into TaskGrid task objects.

TaskGrid task schema (JSON):
${taskSchema}
...
${JSON.stringify(rows, null, 2)}`
  ...
  try {
    ...
    clearTimeout(timer)
    if (e.name === 'AbortError') throw new Error('AI 응답 시간 초과 (120초). 다시 시도해주세요.', { cause: e })
    throw e
  }
  clearTimeout(timer)
  ...
  if (!text) {
    ...
    throw new Error(`Gemini 응답이 비어있습니다 (finish_reason: ${reason || 'unknown'})`)
  }
  ...
  if (start === -1) {
    throw new Error(`Gemini 응답에서 JSON 배열을 찾지 못했습니다. 응답: "${text.slice(0, 200)}"`)
  }

  // Find the best closing bracket — prefer last ] but try repairing truncated JSON
  let jsonStr = stripped.slice(start)
  const end = jsonStr.lastIndexOf(']')
  if (end !== -1) {
    jsonStr = jsonStr.slice(0, end + 1)
  } else {
    // Response was truncated — close open objects and array
  ...
  try {
    ...
    throw new Error(`JSON 파싱 실패: ${e.message}. 응답 앞부분: "${text.slice(0, 200)}"`, { cause: e })
  }
