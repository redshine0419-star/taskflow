...
${JSON.stringify(rows, null, 2)}`
  ...
  let tasks
  try {
    // First try: parse the whole stripped text directly
    ...
    if (start === -1 || end === -1 || end <= start) {
      throw new Error(`Gemini 응답에서 JSON을 찾지 못했습니다. 응답 앞부분: "${text.slice(0, 200)}"`)
    }
    try {
      tasks = JSON.parse(stripped.slice(start, end + 1))
    } catch (e) {
