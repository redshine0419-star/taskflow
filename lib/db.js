...
export async function getFeedback({ page = 1, pageSize = 20 } = {}) {
  ...
  `
  const [{ count }] = await sql`SELECT COUNT(*)::int AS count FROM feedback WHERE is_hidden = false`
  return { rows, total: count, page, pageSize }
}

export async function insertFeedback({ nickname, content, rating, ipHash }) {
  const sql = getDb()
  const [row] = await sql`
    INSERT INTO feedback (nickname, content, rating, ip_hash)
    VALUES (${nickname}, ${content}, ${rating ?? null}, ${ipHash})
    RETURNING id, nickname, content, rating, created_at
  `
  return row
}

export async function countRecentFeedbackByIp(ipHash, since) {
  const sql = getDb()
...
export async function hideFeedback(id) {
  const sql = getDb()
  await sql`UPDATE feedback SET is_hidden = true WHERE id = ${id}`
}
