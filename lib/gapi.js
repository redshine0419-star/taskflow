...
const HEADERS = ['id', 'title', 'stage', 'priority', 'assignee', 'dueDate', 'desc', 'published', 'comments', 'projectId']
...
export async function findOrCreateSpreadsheet() {
  ...
  await sheetsRequest(`/${created.spreadsheetId}/values/${SHEET_TITLE}!A1:J1?valueInputOption=RAW`, 'PUT', {
    values: [HEADERS],
  })
...
export async function loadTasks(spreadsheetId) {
  const data = await sheetsRequest(`/${spreadsheetId}/values/${SHEET_TITLE}!A2:J1000`)
...
export async function updateTaskRow(spreadsheetId, rowNum, task) {
  await sheetsRequest(
    `/${spreadsheetId}/values/${SHEET_TITLE}!A${rowNum}:J${rowNum}?valueInputOption=RAW`,
