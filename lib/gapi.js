...
const BLOG_HEADERS = ['slug', 'title', 'date', 'category', 'desc', 'keywords', 'content', 'usedKeyword']
...
export async function ensureBlogPostsSheet(token, spreadsheetId) {
  ...
  if (!exists) {
    ...
    // Write header row
    await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/BlogPosts!A1:H1?valueInputOption=RAW`,
      {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function appendBlogPost(token, spreadsheetId, post) {
  ...
  const row = [post.slug, post.title, post.date, post.category, post.desc, post.keywords, post.content, post.usedKeyword || '']
  await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/BlogPosts!A:H:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,
    {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
